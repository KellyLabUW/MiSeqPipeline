...
Hash_key <- here("Output", "dada2_output", "run11_20220606", marker, "hash_key.csv")
ASVs <- here("Output","dada2_output","run11_20220606",marker,"ASV_table.csv")
...
## For COI this is to order level (or finer)
####################################################################

# but let's only save the "good ones" (ORDER for COI) to use as a previous effort 
good.clasif.hashes <- clasif.hashes %>% 
  filter(order != "" | family != "" | genus != "" | species != "")
